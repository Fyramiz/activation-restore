...
THIS README WAS GENERATED BY AI, the script uses NO RAMDISK, you just jailbreak with palera1n ``sudo palera1n -l`` and then run an iprcy using ``sudo iproxy 22 44`` (it comes with SSHRD_Script or you can install via the package manager) and then when the phone boots you run ``run.sh``, the device will reboot with activation tickets installed, please note that you can use a passcode and Touch ID without issue. script assumes activation files are in ~/Desktop/Activation, read the full README for other instructions and note NO RAMDISK IS EVER USED.
